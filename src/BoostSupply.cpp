...
static const int MAX_DUTY = ((1000 * 70 * 1024) / 100);
...
void BoostSupply::service()
{
    ...
    // {
    //     ave += samples[i];
    //     ave += adc1_get_raw(m_feedback_pin);
    // }
    // uint32_t value_sense = ave / NUMBER_OF_SAMPLES;
    uint32_t values[NUMBER_OF_SAMPLES];
    for (int i = 0; i < NUMBER_OF_SAMPLES; i++)
    {
        values[i] = adc1_get_raw(m_feedback_pin);
    }
    ...
    int voltage_diff = 600 - feedback_voltage;
